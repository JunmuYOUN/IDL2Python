...
완료 판정은 "테스트 통과"가 아니라 **"원본 IDL 실행 산출물과의 대조 통과(parity PASS)"**다.
...
소스 유형 자동 판별(로컬/Git/웹)과 Phase 0 수집은 `web-source-collector` 스킬을 따른다.
...
**다중 파일(배치) 모드**: 의존성 그룹을 병렬 변환하되, 오라클은 **호출 그래프의 진입점 단위**로 잡는다 (파일마다 IDL을 따로 돌리지 않고, 진입점 1회 실행에 여러 파일의 probe를 함께 심는다).
