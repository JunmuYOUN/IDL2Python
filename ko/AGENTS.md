# AGENTS.md — 에이전트 CLI 진입점

이 저장소는 IDL(`.pro`)→Python 변환을 **실행 기반 수치 대조(parity)**로 검증하는 에이전트 하네스다.
파이프라인 정의(운영 계약 · 역할 · 프로토콜)는 `.claude/` 디렉터리 아래 **순수 마크다운 문서**로,
provider 중립적이며 특정 LLM이나 특정 CLI 전용 형식이 아니다.

이 파일은 어떤 코딩 에이전트 CLI에서든 하네스를 구동하는 방법을 안내한다:

- 일부 CLI(예: Claude Code)는 `.claude/` 디렉터리를 자동 로드하여 아래 절차를 스스로 수행한다.
- 그 외 CLI(예: OpenAI Codex CLI 등 `AGENTS.md`를 읽는 도구)는 아래 절차를 직접 따른다.
